...
const DETAIL_LINK = "https://scrapbox.io/toukatsu-dev/%E3%81%BF%E3%82%93%E3%81%AA%E3%81%A7%E9%9F%B3%E6%A5%BD%E3%83%97%E3%83%AC%E3%82%A4%E3%83%AA%E3%82%B9%E3%83%88%E3%82%92%E4%BD%9C%E3%82%8D%E3%81%86%E3%81%AE%E4%BC%9A";
...
export async function notifyNewMusics() {
    ...
    if (addedTrackIds.length > 0) {
        ...
        if (!tracksDetailRes.ok) {
            ...
            let errorMessage = `HTTP Status: ${tracksDetailRes.status} (${tracksDetailRes.statusText})`;
            try {
                const errorJson = JSON.parse(errorBody);
            ...
            const fields = tracksDetailData.tracks.map((track: SpotifyTrack, index: number) => {
                const trackName = track.name;
                // 複数のアーティストがいる場合があるので、名前をまとめて取得
                const artists = track.artists.map((artist) => artist.name).join(' & ');
                
                console.log(`${index + 1}. 曲名: ${trackName} / アーティスト: ${artists}`);
